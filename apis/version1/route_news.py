...
from db.repository.news import create_news, retreive_news, list_news, update_news_by_id, delete_news_by_id, \
    create_list_news
from moex.client import get_news, get_news_by_id
...
# @router.get("/{id}")
# def read_news(id: int):
#     news = get_news_by_id(id=id)
#     # if not news:
#     #     raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"news with this id {id} does not exist")
#     return news.body


@router.get("/", response_model=List[NewsShow])
def read_news(db: Session = Depends(get_db)):
    create_list_news(news=get_news(), db=db)
